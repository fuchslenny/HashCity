...
if (isset($_REQUEST["completed"]) && $_REQUEST["completed"] !== "") {
    ...
    // Bei leerem Array ist maxLevel = -1, damit das erste erlaubte Level 0 ist
...
<script>
    $(document).ready(function() {
        ...
        // Prüfen ob das ausgewählte Level freigeschaltet ist
        ...
        ?>
        currentLevel = <?php echo $isLevelSelected; ?>;
        <?php
        }
        }  // <- WICHTIG: Schließe den äußeren if-Block
        ?>
        ...
        const levelData = [
            <?php
            $levels = [
                    ['title' => 'Einführung', 'description' => 'Lerne die Grundlagen von Hash-Funktionen'],
                    ['title' => 'Hash-Kollisionen', 'description' => 'Was passiert bei gleichen Hash-Werten?'],
                    ['title' => 'Separate Chaining', 'description' => 'Kollisionen mit verketteten Listen lösen'],
                    ['title' => 'Open Addressing', 'description' => 'Alternative Kollisionsbehandlung'],
                    ['title' => 'Load Factor', 'description' => 'Wann muss die Hash Map wachsen?'],
                    ['title' => 'Rehashing', 'description' => 'Die Hash Map dynamisch vergrößern'],
                    ['title' => 'Optimierung', 'description' => 'Performance-Tuning für Hash-Funktionen'],
                    ['title' => 'Master Challenge', 'description' => 'Baue die perfekte HashCity!']
            ];

            foreach ($levels as $index => $level) {
                $isCompleted = in_array($index, $levelsDone);
                $isUnlocked = $isCompleted || ($index > 0 && in_array($index - 1, $levelsDone));
    ...
    `);
        ...
        function spawnTree() {
            ...
            if (Math.random() < 0.3) {
                setTimeout(() => {
                    createObject(fence);
                }, 500); // Zaun etwas verzögert nach dem Baum
            }
        }
        ...
        // House spawning system 'filled_house.svg', 'empty_house.svg',
        const houseTypesOld = ["Baum1.svg", "Baum2.svg", "Baum3.svg", "WohnhauBlauBraunBesetztNeu.svg", "WohnhauBlauGrauBesetztNeu.svg", "WohnhauBlauRotBesetztNeu.svg", "WohnhauGelbBraunBesetztNeu.svg", "WohnhauGelbRotBesetztNeu.svg", "WohnhauGrauBraunBesetztNeu.svg", "WohnhauGruenBraunBesetztNeu.svg", "WohnhauGruenGrauBesetztNeu.svg", "WohnhauRotRotBesetztNeu.svg", "WohnhausRotBraunBesetztNeu.svg"];



        function createHouseOld() {
            const houseType = houseTypes[Math.floor(Math.random() * houseTypes.length)];
            const duration = 15 + 1 * 1; //12 + Math.random() * 8; // 12-20 Sekunden //Math.random()
            const size = 200;//100 + Math.random() * 100; // 100-200px Breite
        ...
        function startHouseSpawningOld() {
            // Initiale Häuser gleichmäßiger verteilt
            createHouse();
            setTimeout(() => createHouse(), 1500);
            setTimeout(() => createHouse(), 3000);

            // Kontinuierliches Spawning mit kleiner Variation um einen festen Rhythmus
            function scheduleNextHouse() {
                // Basis-Intervall von 2.5s mit nur ±0.5s Variation für Natürlichkeit
                const baseDelay = 2500;
                const variation = 500;
                const delay = baseDelay - variation/2 + Math.random() * variation;
            ...
            // Starte nach dem letzten initialen Haus
            setTimeout(() => {
                scheduleNextHouse();
            }, 3000);
        }
