    ...
    <div class="game-area">
        ...
        <div class="info-panel">
            ...
            <div class="info-item hash-calculator">
                ...
                <input type="text" id="nameInput" class="calculator-input" placeholder="Wähle einen Namen..." readonly>
...
<script>
    $(document).ready(function() {
        ...
        const dialogues = [
            "Willkommen zurück! Gut gemacht bisher. Aber wir haben ein kleines Problem: Das 'Linear Probing' aus dem letzten Stadtteil hat zu viele 'Cluster' erzeugt.",
            "Das bedeutet, dass sich Bewohner an einer Stelle häufen und wir lange nach einem freien Haus suchen müssen. Das ist ineffizient!",
            "Ich habe eine Lösung: <strong>Quadratic Probing</strong>. Damit verteilen wir die Bewohner besser. Aber fangen wir einfach an. Klicke auf 'Levi' in der Liste!"
        ];
        ...
        $('#hashButton').click(function() {
            ...
            // Wer wird berechnet?
            const nameToHash = searchMode ? "Thomas" : selectedFamily;
            if(!nameToHash) return;

            clearMarkers();

            const calcHash = getHash(nameToHash, HASH_SIZE);
            $('#hashResult').text(`Hausnummer: ${calcHash}`);
            $(this).prop('disabled', true); // Deaktivieren -> User muss jetzt Haus klicken

            // Berechnung
            const result = quadraticProbing(nameToHash, HASH_SIZE, stadt);
        ...
        $('.house').click(function() {
            ...
            if (stadt[houseIdx] === null) {
                ...
                // Aufräumen
                clearMarkers();
                $(`.to-do-family[data-family="${selectedFamily}"]`).removeClass('active').addClass('list-group-item-success');
        ...
        function startSearchPhase() {
            ...
            $('#nameInput').val('Thomas').prop('readonly', true);

            // Button wieder freigeben
            $('#hashButton').prop('disabled', false).text('Wo war das?');
